...
db.serialize(function () {
    //teste 1 - terminal
    let stmt = db.prepare("INSERT INTO prninfo (prn, snr, azi, elev, lat, long, time) VALUES(?,?,?,?,?,?,?)");
    ...
    gps.on('data', function (data) {
        ...
        if (data.msgNumber !== undefined && data.msgNumber !== 'null') {
            for (const element of data.satellites) {
                stmt.run(element.prn, element.snr, element.azimuth,
                    element.elevation, latitude, longitude, time);
            }
        }
        ...
        if (time.getUTCSeconds() === 0 && time.getMinutes() !== controle) {
            ...
            let stmt2 = ("select prn, count(snr) as total from prninfo where time between ?-60000 and ? group by prn");
                ...
                if (err) {
                    ...
                    rows.forEach((row) => {
                        ...
                        if (row.total >= MIN_QTDE) {
                            ...
                            let sql = "SELECT prn, snr FROM prninfo where time between ?-60000 and ? and prn = ?";
                            let vSnr = [];
                            ...
                            db.all(sql, [time, time, row.prn], (err, rows) => {
                                ...
                                let mediaIntensidadeSinalQuadrado = (mean(vIntensidadeSinal));
                                mediaIntensidadeSinalQuadrado = pow(mediaIntensidadeSinalQuadrado,2);
                                let s4 = sqrt( ((intensidadeSinalQuadrado-mediaIntensidadeSinalQuadrado) / mediaIntensidadeSinalQuadrado) );
                                
                                //sallet na tabela prnindices
                                let stmt3 = ("INSERT INTO prnindices (prn, mediasnr, mediaazi, mediaelev, tinicial, tfinal, dpsnr, s4) SELECT prn, " +
